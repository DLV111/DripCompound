...
class DripCompoundClass:
    def __init__(
        ...
        **kwargs
        ...
        self.drip_contract = self.w3.eth.contract(
            to_checksum(DRIP_TOKEN_ADDRESS), abi=read_json_file(DRIP_FAUCET_ABI_FILE)
        )
    ...
    def read_in_config(self):
        """Reads in the config from the config file"""
        config_file = self.config_args["config_file"]
        if self.config_args["new_config"] is True:
            self.create_default_config(config_file)
        try:
            config = configparser.ConfigParser(
                {
                    "min_bnb_balance": False,
            ...
            logging.info(
                "If this config file does not exist yet, run with -n to create"
            )
        ...
        if os.path.exists(config_file):
            ...
            try:
                with open(config_file, "w") as f:
                    config.write(f)
                    logging.info(
                        "A new template file has been created at '%s'. Please review and update",
                        config_file,
                    )
                    sys.exit(0)
            except IOError:
    ...
    def argparser(self):
        ...
        parser.add_argument(
            ...
            "--new-config",
            action="store_true",
            help="Create a new config file at the location specified - If file exists no action is taken",
        )
    ...
    def getAvailableClaims(self):
        self.claimsAvailable = round(
            wei2eth(self.drip_contract.functions.claimsAvailable(self.address).call()),
            self.rounding,
        )
    ...
    def checkAvailableBNBBalance(self):
        ...
        if self.min_bnb_balance:  # Do we have a min balance defined?
            if self.BNBbalance < self.min_bnb_balance:
                msg = (
                    "Your current BNB balance(%s) is below min required (%s) for %s"
                    % (self.BNBbalance, self.min_bnb_balance, self.wallet_friendly_name)
                )
                logging.info(msg)
        ...
        if self.perform_drip_compounding.lower() == "true":
            for _ in range(max_tries):
                try:
                    ...
                    if (
                        txn_receipt
                        and "status" in txn_receipt
                        and txn_receipt["status"] == 1
                    ):
                        logging.info("Transaction Successful: %s", (self.w3.toHex(txn)))
                        time.sleep(default_sleep_between_actions)
                        self.getDripBalance()
                        logging.info(
                            "Updated Drip balance is: %s (Increase %s) - tx %s",
                            self.DripBalance,
                            self.getDripBalanceIncrease(),
                            self.w3.toHex(txn),
                        )
                        self.sendMessage(
                            "Compounding Complete",
                            "Updated Balance %s (Increase %s) - tx %s"
                            % (
                                self.DripBalance,
                                self.getDripBalanceIncrease(),
                                self.w3.toHex(txn),
                        ...
                        logging.info(
                            "Compounding Failed. %s retries remaining (%s seconds apart). Transaction status '%s' - tx %s",
                            remaining_retries,
                            retry_sleep,
                            txn_receipt["status"],
                            self.w3.toHex(txn),
                        ...
                        self.sendMessage(
                            "Compounding Failed",
                            "%s retries remaining (%s seconds apart). Transaction status '%s' - tx %s"
                            % (
                                remaining_retries,
                                retry_sleep,
                                txn_receipt["status"],
            ...
            self.getDripBalance()
            logging.info(
                "Updated Drip balance is: %s (Increase %s)",
                self.DripBalance,
                self.getDripBalanceIncrease(),
            )
            self.sendMessage(
    ...
    def PushOverClientInit(self):
        if self.pushover_api_key and self.pushover_user_key:
            self.client = Client(
                self.pushover_user_key, api_token=self.pushover_api_key
            )
...
def main():
    ...
    dripwallet.sendMessage(
        "Drip Compounding",
        "Current Balance %s - Compound %s"
        % (dripwallet.DripBalance, dripwallet.claimsAvailable),
    )
